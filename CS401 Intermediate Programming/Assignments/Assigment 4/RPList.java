...
public class RPList
{
	...
	public void saveList()
	{
		try
		{
			...
			for(int i = 0; i < size; i++)
			{
        		c.append(plist[i].name + "," + plist[i].password + "," + plist[i].getMoney() + "," + plist[i].getDebt());
        		if(plist[i].userQuestions != null)
        		{
        			c.append(",");
        			c.append(plist[i].userQuestions[0].getQ() + "," + plist[i].userQuestions[0].getA() + "," + plist[i].userQuestions[1].getQ()
	...
	//returns RoulettePlayer object
	public RoulettePlayer getPlayer(String name)
	{
		for(int i = 0; i < plist.length; i++)
		{
			if(plist[i].name.equals(name))
