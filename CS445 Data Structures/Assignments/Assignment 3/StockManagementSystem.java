 ...
 public class StockManagementSystem{
	...
	public int getNumberOfStockedItemsInCategory(String categoryName) {
		...
		for(int i = 0; i < size; i++)
		{
			...
			if(c.getCategoryName().equals(categoryName))
			{
				stock = c.getTotalQuantityOfStock();
			}
		}
		return stock;
	}
	...
	public int getNumberOfStockedItemsInCategories(ListInterface<String> categoryNames) {
		...
		int size = categories.getSize();
		int total = 0;
		int sizetwo = categoryNames.getSize();
		for(int i = 0; i < size; i++)
		{
			Category c = categories.get(i);
			if(categoryNames.contains(c.getCategoryName())) //categoryNames.contains(c.getCategoryName())
			{
				total += c.getTotalQuantityOfStock();
			}
		}
		return total;
	}
	...
	public int getQuantityOfItemByName(String productName) {
		...
		{
			Category c = categories.get(i);
			Product p = c.findProductByName(productName);
			if(p != null)
			{
				return p.getQuantityInStock();
			}
		}
		return 0;
	}
	...
	public Product removeProductByName(String productName) { 
		...
		{
			Category c = categories.get(i);
			c.removeProductByName(productName);
		}
		// productName.removeProductByName(productName);
		return null;
	}
