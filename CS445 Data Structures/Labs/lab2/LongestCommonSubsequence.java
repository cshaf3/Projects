...
public class LongestCommonSubsequence {
    ...
    public static void main(String args[]) {
        ...
        // algorithm
        while(possibleSubsequences.isEmpty() == false)
        {
            ...
            if(longest.length() < test.length())
            {
                ...
                else if((test.length()-2) > longest.length())
                {
                    for(int i = 0; i < test.length(); i++)
                    {
                        char s = test.charAt(i);
                        String characterOfTest = String.valueOf(s); 
                        possibleSubsequences.add(characterOfTest);
                    }
                }
                else
                {
                    System.out.print(possibleSubsequences.toString());
                }
            }
